...
public partial class LevelGenerator : Node
{
    ...
    public void GenerateLevel(RoomSet set, int width, int height, ulong seed)
    {
        ...
        do
        {
            ...
            {
                var stack = new Stack<GeneratorRoom>();
                // grab lowest entropy
                var lowestEntropy = state.Cast<GeneratorRoom>().Where(room => !room.IsCollapsed)
                    .Min(room => room.GetEntropy());
                // pick random room with lowest entropy
                var lowEntRoom = state.Cast<GeneratorRoom>()
                    .Where(room => !room.IsCollapsed && room.GetEntropy() == lowestEntropy)
                    .ToList()[
                        random.RandiRange(0,
                            state.Cast<GeneratorRoom>()
                                .Count(room => !room.IsCollapsed && room.GetEntropy() == lowestEntropy) - 1)];
                ...
                foreach (var neighbor in GetNeighbors(lowEntRoom, state)) stack.Push(neighbor);
        ...
        for (var j = 0; j < height; j++)
        {
            ...
            label.LabelSettings.OutlineColor = new Color(0, 0, 0);
            ...
            foreach (var side in Enum.GetValues(typeof(RoomDef.Side)).Cast<RoomDef.Side>())
            {
                ...
                sidelabel.LabelSettings.OutlineColor = new Color(0, 0, 0);
    ...
    private static GeneratorRoom[,] DefaultState(RoomSet set, int width, int height)
    {
        ...
        for (var i = 0; i < width; i++)
        for (var j = 0; j < height; j++)
            state[i, j] = new GeneratorRoom(set.Rooms, new Vector2I(i, j));
        return state;
    }
